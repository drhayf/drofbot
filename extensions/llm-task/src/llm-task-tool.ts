...
async function loadRunEmbeddedPiAgent(): Promise<RunEmbeddedPiAgentFn> {
  ...
  try {
    const mod = await import("../../../src/brain/agent-runner/pi-embedded-runner.js");
...
export function createLlmTaskTool(api: OpenClawPluginApi) {
  return {
    name: "llm-task",
    description:
      "Run a structured JSON-only LLM task with optional schema validation. " +
      "Use for: structured data extraction, classification, analytical scoring, " +
      "multi-criteria evaluation, generating structured opinions, or any task " +
      "where you need clean JSON output (not free-form text). " +
      "Works standalone or via Lobster workflow orchestration.",
