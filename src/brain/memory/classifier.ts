...
import { extractJSON, validateClassification } from "../shared/llm-output-validator.js";
...
export class MemoryClassifier {
   ...
   * Uses the shared validator for robust extraction and corruption detection.
   */
  private parseResult(raw: string): ClassificationResult {
    // Use shared validator for robust JSON extraction
    const parsed = extractJSON(raw);
    if (!parsed) {
      log.warn(`Failed to extract JSON from classification response: ${raw.slice(0, 200)}`);
      return { shouldStore: false, banks: [] };
    }

    // Use shared validator for shape validation and corruption detection
    const validated = validateClassification(parsed);
    if (!validated) {
      log.warn(`Classification validation failed or returned corrupted data: ${raw.slice(0, 200)}`);
      return { shouldStore: false, banks: [] };
    }

    if (!validated.shouldStore) {
      return { shouldStore: false, banks: [] };
    }

    // Convert validated result to ClassificationEntry format
    const validBanks = new Set<string>(["episodic", "semantic", "procedural", "relational"]);
    const banks: ClassificationEntry[] = validated.banks
      .filter((b) => validBanks.has(b.name))
      .map((b) => ({
