...
describe("buildAgentSystemPrompt", () => {
  ...
  it("includes capability awareness section when tools are available", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: [
        "memory_search",
    ...
    expect(prompt).toContain("## Proactive Capability Awareness");
    expect(prompt).toContain("### Memory");
    expect(prompt).toContain("search memory first");
    expect(prompt).toContain("Structured memory banks");
    expect(prompt).toContain("### Cosmic Council");
    expect(prompt).toContain("cosmic_synthesis");
    expect(prompt).toContain("### Profiles");
    expect(prompt).toContain("profile_explore");
    expect(prompt).toContain("### Structured Analysis (llm-task)");
  });

  it("omits capability awareness section in minimal mode", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      promptMode: "minimal",
      toolNames: ["memory_search", "cosmic_synthesis", "llm-task"],
    });

    expect(prompt).not.toContain("## Proactive Capability Awareness");
  });

  it("omits capability subsections when tools are not available", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/openclaw",
      toolNames: ["read", "write", "exec"],
    });

    // Section header should still appear (empty section with just header + blank line)
    expect(prompt).toContain("## Proactive Capability Awareness");
    expect(prompt).not.toContain("### Memory");
    expect(prompt).not.toContain("### Cosmic Council");
    expect(prompt).not.toContain("### Profiles");
    expect(prompt).not.toContain("### Structured Analysis");
  });
});
