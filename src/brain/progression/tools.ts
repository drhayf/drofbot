...
import type { Quest, QuestDifficulty, QuestStatus, QuestType } from "./types.js";
...
const QuestCreateSchema = Type.Object({
  ...
  questType: Type.Optional(
    Type.Unsafe<string>({
      type: "string",
      enum: ["daily", "cosmic", "growth", "personal", "weekly"],
      description: "Classification of the quest. Default is personal.",
    }),
  ),
  cosmicAlignment: Type.Optional(
    Type.String({
      description: "Optional elemental, planetary, or astrological theme to tag this quest with.",
    }),
  ),
...
export function createQuestCreateTool(): AnyAgentTool {
  return {
    ...
    execute: async (_toolCallId, params) => {
      ...
      const questType = (readStringParam(params, "questType") as QuestType) || "personal";
      const cosmicAlignment = readStringParam(params, "cosmicAlignment");
      ...
      const quest = engine.createQuest({
        ...
        questType,
        difficulty,
        source: "agent", // Now generated by the intelligence explicitly
        cosmicAlignment,
        expiresAt,
      });
